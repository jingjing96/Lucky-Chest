...
- `/function draw:op/clear_up/draw_items`清除所有正在参与抽奖的青金石块物品实体；
- `/function draw:op/clear_up/chest_items`清除所有宝箱物品实体；
- `/function draw:op/clear_up/clear_all`清除所有残留实体；
...
许愿井目前共有15个变种，每种井的搭建材料和适用的群系/结构已在下表中列出[^1]：

| 种类    | 基础材料                 | 井底中心方块 | 井内流体 | 适用群系/结构                          |
| ----- | -------------------- | ------ | ---- | -------------------------------- |
| 村庄井   | 砖块                   | 磁石     | 水源   | 村庄[^2]、林地府邸、掠夺者前哨站、繁花森林          |
...
| 恶地井   | 红砂岩                  | 磁石     | 水源   | 恶地类群系                            |
| 苔石井1  | 苔石                   | 磁石     | 水源   | 丛林神庙、YUNG的更好的地牢<br/>（原版地牢不行[^3]） |
| 苔石井2  | 苔石                   | 磁石     | 细雪   | 雪屋                               |
...
[^1]: 许愿井只能使用“基础材料”一列中写明的方块搭建，使用其他变种（例如切制砂岩）则不行。
[^2]: 只能是自然生成的村庄（即可以使用/locate指令定位到的），玩家建造的村庄不行。
[^3]: 由于原版地牢不属于“结构”，无法使用/locate指令定位，所以无法方便地判断玩家是否处于地牢中。
